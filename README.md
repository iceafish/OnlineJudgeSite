...
###5.24 icefish
---------------
	为了win下运行方便，在项目根目录添加运行bat
	重新架构Judge后已完成JRequest.views
	重写Judge部分中
	BTW:  通化邀请赛加油~
...
###5.23 icefish
---------------
	当代码提交后，由前台的views.submit_code 函数响应，提交合法后
	将相关信息存入数据库RequestList后，返回views(Main).status
	
	判题进程重新设计成两个
	1.ReqeustQue Listenner
	2.Judger Server
	
	第一个进程是用来监听数据库，设计一个当前最后一个处理完毕的下界
		每经过一段时间，将数据库中处于未判定的数据添加入队列(同步队列)
		(队列操作需加锁)
	
	第二个进程用来每次循环的时候，等待同步队列，如果有等待，那么提取一个请求
		判定结束以后，通过判定结果对数据库进行修改
		
		
	根据judge重构需要对涉及到的模型和应用进行了重构。
	根据judge重构需要，对部分目录结构进行了重构。
